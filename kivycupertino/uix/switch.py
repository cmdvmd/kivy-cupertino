...
<CupertinoSwitch>:
    ...
    Widget:
        id: thumb
        width: root.width/2
        height: root.height-(root.width*0.03)
        y: root.y+(root.height/2)-(self.height/2)
        x: root.x+(root.width*0.03)
    ...
    **KV**
    ...
    def on_toggled(self, instance, value):
        if value:
            ...
            thumb_animation = Animation(x=self.x+self._thumb.width-(self.width*0.03), duration=self.switch_duration)
        else:
            color_animation = Animation(_background_color=self.color_untoggled, duration=self.switch_duration)
            thumb_animation = Animation(x=self.x+(self.width*0.03), duration=self.switch_duration)
